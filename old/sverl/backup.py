...
class PolicyCharacteristic(nn.Module):
    ...
    def train(self, data: TensorDataset, writer: SummaryWriter):
        ...
        EPOCHS = 1000
        for epoch in tqdm(range(1 + self.epochs_completed, EPOCHS + 1), initial=self.epochs_completed, total=EPOCHS):

            # Loop through mini batches
            for observations, action_probs in train_loader:
                # For each mini batch:
                # - Get mask
