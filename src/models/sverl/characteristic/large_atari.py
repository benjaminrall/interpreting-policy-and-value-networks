...
class CharacteristicLargeAtari(Model):
    """Large characteristic function model for SVERL in Atari environments."""
    ...
    def _construct_model(self, output_size) -> nn.Module:
        model = nn.Sequential(
            ...
            nn.ReLU(),\
            self._init_layer(nn.Conv2d(128, 128, 3, stride=1)),
            nn.ReLU(),
            nn.Flatten(),
            self._init_layer(nn.Linear(128 * 7 * 7, 1024)),
            nn.ReLU(),
